...
/*   Updated: 2023/09/15 18:45:41 by jaeyojun         ###   ########seoul.kr  */
/*                                                                            */
/* ************************************************************************** */
...
void	set_dda(t_game *game, t_ray *ray, int *x)
{
	...
	if (ray->raydir_x < 0)
		...
		ray->distance_x = (ray->intpos_x + 1.0 - \
			game->player->pos_x) * ray->delta_x;
	if (ray->raydir_y < 0)
		ray->distance_y = (game->player->pos_y - ray->intpos_y) * ray->delta_y;
	else
		ray->distance_y = (ray->intpos_y + 1.0 - \
			game->player->pos_y) * ray->delta_y;
	return ;
}
...
void	run_dda(t_game *game)
{
	...
	while (++x < WIN_WIDTH)
	{
		...
		while (ray.hit == 0)
		{
			...
			if (game->map_copy[ray.intpos_x][ray.intpos_y] > '0')
				ray.hit = 1;
		}
	}
}
